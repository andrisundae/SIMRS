  ...
  try {
    ...
    if (response.status) {
      if (response.data.isValidLogin) {
        ...
        if (
          response.info.modul === 'auth' &&
          response.info.note === 'force-login'
        ) {
...
function* handleLoadPermissions({ meta }) {
  try {
    ...
    if (response.status) {
      yield put(
        aclActions.getGranted.requestSuccess(meta.resource, response.data)
      );
    } else {
      yield put(
        aclActions.getGranted.requestFailure(meta.resource, response.message)
      );
    }
  } catch (e) {
