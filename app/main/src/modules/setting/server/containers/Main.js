  ...
  render() {
    return (
      <Modal open={this.props.open} onClose={this.props.onClose} size="tiny">
        <Modal.Header>
          <Icon name="setting" /> Atur Alamat Server
        </Modal.Header>
        <Modal.Content>
          <Form>
            <Form.Field
              error={this.state.errors.server ? true : false}
              required
            >
              <label>Server</label>
              <input
              ...
              {this.state.errors.server && (
                <Label basic color="red" pointing>
                  {_.first(this.state.errors.server)}
                </Label>
              )}
            </Form.Field>
          </Form>
    ...
    if (_.isEmpty(errors)) {
      if (this._isValidURL(server)) {
        ...
        messageBox({
          title: 'Informasi',
          message:
            'Alamat server berhasil diganti, klik tombol OK untuk mereload halaman.',
          onOk: () => {
            this.props.onClose();
