...
class Import extends Component {
    constructor(props) {
        ...
        this.rws = new ReconnectingWebSocket(process.env.REACT_APP_SOCKET_SERVER_DOMAIN);
    }
        ...
        onUpload(post, {
            ...
            (error, res, body) => {
                if (error || res.statusCode >= 400) {
                    action.onImportFailure(resource, t(this._getKey('import.message.failure'), { error: error.message}));
                    ...
                    if (body.status) {
                        ...
                        action.onImportFailure(resource, t(this._getKey('import.message.failure'), { error: body.message }));
        ...
        if (response.data) {
            ...
            switch (data.status) {
                case 'onProgress':
                    ...
                    break;
                case 'onError':
                    action.onImportFailure(resource, t(this._getKey('import.message.failure'), { error: data.message }));
                    this._resetState();
                    // this.props.action.onReset(this.props.resource);
                    toastr.error('Error', data.message);
                    break;
                case 'onSuccess':
                    action.onImportSuccess(resource, t(this._getKey('import.message.success'), { total: data.source.total_row }));
                    this._resetState();
                    // this.props.action.onReset(this.props.resource);
