...
class DataTableServerSide extends Component {
                    ...
                    this.props.contextMenuItems.map((item, index) => {
                        return (() => {
                            if (item === alias.contextMenu.separator) {
                                ...
                                return (
                                    <Item key={index} onClick={() => { item.action(this.gridApi, this.state.focusData) }}>
                                        <div className="row">
    ...
    _navigateToNextCell (params) {
        ...
        if (suggestedCellNode) {
            rowIndex = suggestedCellNode.isSelected() ? previousCell.rowIndex : rowIndex;
    ...
    _onSuppressKeyboardEvent(params) {
        let { keyDown, keyUp, keyRight, keyLeft, escape, enter, space } = alias.keyBoard;
        let keysToSuppress = [
            keyDown, keyUp, keyRight, keyLeft, escape, enter, space
        ];
