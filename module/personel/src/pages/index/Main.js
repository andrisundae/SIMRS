...
import {
  ...
  authActions,
  getPermissions,
} from '@simrs/main/src/modules/auth';
...
class Main extends Component {
  ...
  _bindKey() {
    ...
    MouseTrap.bindGlobal('alt+n', function (e) {
      e.preventDefault();
      if (
        _this.props.customPermissions.canChangeStatus &&
        !_this.props.isDisableForm &&
        _this.props.isRowSelected
      ) {
    ...
    MouseTrap.bindGlobal('alt+e', function (e) {
      e.preventDefault();
      if (
        _this.props.customPermissions.canResetPassword &&
        !_this.props.isDisableForm &&
        _this.props.isRowSelected
      ) {
    ...
    MouseTrap.bindGlobal('alt+l', function (e) {
      e.preventDefault();
      if (
        _this.props.customPermissions.canForceLogout &&
        !_this.props.isDisableForm &&
        _this.props.isRowSelected
      ) {
  ...
  _onChangeStatus = () => {
    const {
      post: { nama, nip, string_aktif },
      t,
      resource,
    } = this.props;
    let subActionMessage =
      string_aktif === 'Ya' ? 'menonaktifkan' : 'mengaktifkan';
    confirmation({
      onOk: () =>
        this.props.action.changeStatus(this.resource, {
          id: this.props.selectedRow,
        }),
      message: t(`${resource}:dialog.confirmation.change_status`, {
        subActionMessage,
        nama,
        nip,
      }),
  ...
  _onResetPassword = () => {
    ...
    confirmation({
      onOk: () =>
        this.props.action.resetPassword(this.resource, {
          id: this.props.selectedRow,
        }),
      message: t(`${resource}:dialog.confirmation.reset_password`, {
        nama,
        nip,
      }),
    });
  ...
  _onForceLogout = () => {
    ...
    confirmation({
      onOk: () =>
        this.props.action.forceLogout(this.resource, {
          id: this.props.selectedRow,
        }),
  ...
  getColumnDefs() {
    return [
      ...
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.jenis_kelamin`
        ),
        field: 'jenis_kelamin',
        width: 140,
      },
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.pendidikan`
        ),
        field: 'pendidikan',
        width: 120,
      },
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.jenis_pegawai`
        ),
        field: 'jenis_pegawai',
        width: 140,
      },
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.spesialisasi_pegawai`
        ),
      ...
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.jabatan_fungsional`
        ),
      ...
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.status_aplikasi`
        ),
      ...
      {
        headerName: this.props.t(
          `${this.props.resource}:header.column.username`
        ),
        field: 'username',
        width: 120,
      },
  ...
  render() {
    ...
    return (
      <Segment size="mini">
        ...
        {isShowUploadGambar && (
          <UploadGambar t={t} i18n={i18n} resource={resource} />
        )}
        <FooterActions
          t={t}
          resource={resource}
