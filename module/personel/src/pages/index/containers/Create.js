...
import { isDisableForm, moduleActions, moduleActionTypes } from '@simrs/main/src/modules/master/default';
...
class Create extends Component {
        ...
        return (
            <Form id={this.formId} size="small">
                <Grid>
                    ...
                    <Grid.Row className="form-row">
                        ...
                        <Grid.Column width="11" className="field">
                            <Select
                                ...
                                onChange={(selected) => this._handleSelect2Change('id_jenis_kelamin', selected)}
                    ...
                    <Grid.Row className="form-row">
                        ...
                        <Grid.Column width="11" className="field">
                            <Select
                                ...
                                onChange={(selected) => this._handleSelect2Change('id_pendidikan', selected)}
                    ...
                    <Grid.Row className="form-row">
                        ...
                        <Grid.Column width="11" className="field">
                            <Select
                                ...
                                onChange={(selected) => this._handleSelect2Change('id_jenis_pegawai', selected)}
                                onKeyDown={(e) => this._onFocusElement(e, 'id_spesialisasi_pegawai')}
                    ...
                    <Grid.Row className="form-row">
                        <Grid.Column width="5" className="required field">
                            <label>{t(this._getKey('label.field.spesialisasi_pegawai'))}</label>
                        </Grid.Column>
                        <Grid.Column width="11" className="field">
                            <Select
                                name="id_spesialisasi_pegawai"
                                placeholder={t(this._getKey('placeholder.field.spesialisasi_pegawai'))}
                                inputRef={this.id_spesialisasi_pegawai}
                                isDisabled={isDisableForm}
                                value={this._getSelect2Value('id_spesialisasi_pegawai')}
                                onChange={(selected) => this._handleSelect2Change('id_spesialisasi_pegawai', selected)}
                                onKeyDown={(e) => this._onFocusElement(e, 'id_jabatan_fungsional')}
                    ...
                    <Grid.Row className="form-row">
                        ...
                        <Grid.Column width="11" className="field">
                            <Select
                                ...
                                placeholder={t(this._getKey('placeholder.field.jabatan_fungsional'))}
                                inputRef={this.id_jabatan_fungsional}
                                isDisabled={isDisableForm}
                                value={this._getSelect2Value('id_jabatan_fungsional')}
                                onChange={(selected) => this._handleSelect2Change('id_jabatan_fungsional', selected)}
                                onKeyDown={(e) => this._onFocusElement(e, 'alias_status_aplikasi')}
                    ...
                    <Grid.Row className="form-row">
                        <Grid.Column width="5" className="required field">
                            <label>{t(this._getKey('label.field.alias_status_aplikasi'))}</label>
                        </Grid.Column>
                        <Grid.Column width="11" className="field">
                            <Select
                                name="alias_status_aplikasi"
                                placeholder={t(this._getKey('placeholder.field.alias_status_aplikasi'))}
                                inputRef={this.alias_status_aplikasi}
                                isDisabled={isDisableForm}
                                value={this._getSelect2Value('alias_status_aplikasi')}
                                onChange={(selected) => this._handleSelect2Change('alias_status_aplikasi', selected)}
                                onKeyDown={(e) => this._onFocusElement(e, this._isNotPelaksana() ? 'username' : 'grup_0')}
                        ...
                        <Grid.Row className="form-row">
                            ...
                            <Grid.Column width="11" className="field">
                                <Input
                                    ...
                                    onChange={this._handleInputChange}
                                    onKeyDown={
                                        (e) => this._onFocusElement(e, statusForm === moduleActionTypes.ADD ? 'password' : 'grup_0')
                                    }
                                    maxLength={70}
                                />
                            </Grid.Column>
                        ...
                        <Grid.Row className="form-row">
                            ...
                            <Grid.Column width="11" className="field">
                                <Input
                                    ...
                                    ref={this.password}
                                    value={post.password}
                                    disabled={statusForm === moduleActionTypes.ADD ? false : true}
                                    onChange={this._handleInputChange}
                                ...
                                return (
                                    <Grid.Column key={grup.id}>
                                        <Checkbox
                                            ...
                                            checked={isSelected}
                                            disabled={isDisableForm}
                                            onChange={(e) => this._handleGrupChange(e, isSelected, index)}
                                            inputRef={input => this["grup_" + index] = input}
                                            onKeyDown={(e) => this._onFocusElement(e, (this._countGroup() - 1) === index ? 'save' : `grup_${index + 1}`)}
                                            label={grup.nama}
                                        />
                                    </Grid.Column>
                                )
    ...
    _isNotPelaksana() {
        ...
        return post.alias_status_aplikasi && post.alias_status_aplikasi.toUpperCase() !== 'PELAKSANA' ? true : false;
    }
        ...
        if (selectedValue) {
            switch (element) {
                case 'id_jenis_kelamin': 
                    value = this.props.optionsJenisKelamin.find(row => row.value === selectedValue);
                    break;
                case 'id_pendidikan':
                    value = this.props.optionsPendidikan.find(row => row.value === selectedValue);
                    break;
                case 'id_jenis_pegawai':
                    value = this.props.optionsJenisPegawai.find(row => row.value === selectedValue);
                    break;
                ...
                case 'id_jabatan_fungsional':
                    value = this.props.optionsJabatanFungsional.find(row => row.value === selectedValue);
                    break;
                case 'alias_status_aplikasi':
                    value = this.props.optionsStatusAplikasi.find(row => row.value === selectedValue);
                    break;
                default:
                    break;
            }
    ...
    _handleGrupChange(e, isSelected, index) {
        ...
        this.props.action.onGrupChange(this.props.resource, { value: parseInt(value), index, isSelected });
    }
