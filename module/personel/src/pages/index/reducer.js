...
const moduleReducer = (state = moduleState, action) => {
  ...
  switch (type) {
    case actionTypes.POPULATE_FORM_SUCCESS:
      return produce(state, (draft) => {
        let {
          ...
          spesialisasi_pegawai: spesialisasiPegawai,
          jabatan_fungsional: jabatanFungsional,
          status_aplikasi: statusAplikasi,
          grups,
        } = payload.data;
    ...
    case actionTypes.CHANGE_GRUP: {
      ...
      return produce(state, (draft) => {
        if (data.isSelected) {
          draft.post.grups = state.post.grups.filter(
            (row) => row !== data.value
          );
        } else {
          draft.post.grups = [...state.post.grups, data.value];
        }
  ...
  switch (type) {
    ...
    case actionTypes.CHANGE_JENISGAMBAR:
      return produce(state, (draft) => {
        ...
        draft.post.id_jenis_gambar_personel = payload.data
          ? payload.data.value
          : '';
      });
    ...
    case actionTypes.GET_UPLOADGAMBAR_DETAIL_SUCCESS:
      return produce(state, (draft) => {
        if (payload.data && payload.data.id) {
          ...
          draft.post.id_jenis_gambar_personel =
            payload.data.id_jenis_gambar_personel;
