...
} from '@simrs/main/src/modules/auth/authActions';
...
function* handleChangeStatus({ payload, meta }) {
  try {
    ...
    if (response.status) {
      yield put(
        actions.changeStatusAplikasi.requestSuccess(meta.resource, response)
      );
    } else {
      yield toastr.error(response.message);
    }
...
function* handleResetPassword({ payload, meta }) {
  try {
    ...
    if (response.status) {
      yield put(
        authActions.resetPassword.requestSuccess(meta.resource, response)
      );
...
function* handleForceLogout({ payload, meta }) {
  try {
    ...
    if (response.status) {
      yield put(
        authActions.forceLogout.requestSuccess(meta.resource, response)
      );
      yield toastr.success(response.message);
    } else {
      yield toastr.error(response.message);
...
function* handleLoadPermissions({ meta }) {
  try {
    ...
    let resetPassword = yield call(
      apiAuth.getGranted,
      '_auth_personel_password'
    );
...
function* openFormUploadGambar({ meta, payload }) {
  ...
  yield put(
    uploadGambarActions.detail.request(meta.resource, {
      id_personel: payload.data.id,
    })
  );
}
...
export default function* watchActions() {
  yield all([
    ...
    takeLatest(
      filterIndexActionTypes.FILTER_CHANGE_STATUS_APLIKASI,
      handleFilterStatusAplikasi
    ),
    ...
    takeLatest(
      ...
      handleDeleteUploadGambar
    ),
    takeLatest(
      actionTypes.DELETE_UPLOADGAMBAR_SUCCESS,
      handleDeleteUploadGambarSuccess
    ),
  ]);
}
