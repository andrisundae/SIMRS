...
function* populateForm({ meta }) {
  try {
    ...
    if (instalasi.status) {
      yield put(
        getOptionsInstalasi.requestSuccess(meta.resource, instalasi.data)
      );
    } else {
      yield put(
        getOptionsInstalasi.requestFailure(meta.resource, instalasi.message)
      );
    }
    ...
    if (kategori.status) {
      yield put(
        getOptionsKategori.requestSuccess(meta.resource, kategori.data)
      );
    } else {
      yield put(
        getOptionsKategori.requestFailure(meta.resource, kategori.message)
      );
    }
