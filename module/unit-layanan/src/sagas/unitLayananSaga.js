...
import api from '../services/models/unitLayananModel';
import {
    moduleActionTypes,
    moduleActions,
    filterActionTypes
...
function* handleAdd({ meta }) {
    yield put(moduleActions.onFocusElement(meta.resource, 'instalasi'));
}
...
function* populateForm({ meta }) {
    ...
    try {
        ...
        let { getOptionsInstalasi, getOptionsKategori } = actions;

        let instalasi = yield call(api.getOptionsInstalasi);
        if (instalasi.status) {
            yield put(getOptionsInstalasi.requestSuccess(meta.resource, instalasi.data));
        } else {
            yield put(getOptionsInstalasi.requestFailure(meta.resource, instalasi.message));
        }

        let kategori = yield call(api.getOptionsKategori);
        if (kategori.status) {
            yield put(getOptionsKategori.requestSuccess(meta.resource, kategori.data));
        } else {
            yield put(getOptionsKategori.requestFailure(meta.resource, kategori.message));
        }
