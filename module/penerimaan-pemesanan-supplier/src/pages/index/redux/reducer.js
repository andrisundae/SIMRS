...
const masterReducer = (state = masterState, action) => {
  ...
  switch (type) {
    ...
    case actionTypes.SET_PEMESANAN:
      return produce(state, (draft) => {
        draft.post.no_pemesanan = payload.data.no_transaksi;
        draft.post.tanggal_pemesanan = dateFormatClient(
          payload.data.tanggal_transaksi,
          'DD/MM/YYYY HH:mm:ss'
        );
        draft.post.supplier = payload.data.nama_supplier;
        draft.post.unit_penerima = payload.data.nama_unit;
        draft.post.id_pemesanan = payload.data.id;
      });
    case masterActionTypes.SET_DATA_MASTER:
      return produce(state, (draft) => {
        ...
        draft.post.id_pemesanan = payload.data.id_pemesanan;
        draft.post.no_pemesanan = payload.data.no_pemesanan;
        draft.post.tanggal_pemesanan = dateFormatClient(
          payload.data.tanggal_pemesanan
        );
        draft.post.supplier = payload.data.nama_supplier;
        draft.post.unit_penerima = payload.data.nama_unit;
...
const detailReducer = (state = detailState, action) => {
  ...
  switch (type) {
    case detailActionTypes.SET_DATA_DETAIL:
      return produce(state, (draft) => {
        draft.post.kode_barang = payload.data.barcode;
        draft.post.nama_barang = payload.data.nama;
        draft.post.id_barang = payload.data.id;
        draft.post.harga_satuan = payload.data.harga_terakhir;
        draft.statusForm = detailActionTypes.EDIT;
      });
    case actionTypes.SET_INFO_BARANG:
      return produce(state, (draft) => {
        let jumlah_terima =
          parseFloat(payload.data.jumlah_beli_sebelumnya) +
          parseFloat(payload.data.jumlah_beli_sekarang);

        draft.post.satuan_terkecil = payload.data.nama_satuan_terkecil;
        draft.post.jumlah_pesan = payload.data.jumlah_pesan;
        draft.post.jumlah_terima_sbl = jumlah_terima;
      });
...
const filterReducer = (state = filterState, action) => {
  ...
  switch (type) {
    case actionTypes.INIT_FORM:
      return produce(state, (draft) => {
        draft.data.initial.option_filter_pemesanan =
          payload.data.option_filter_pemesanan;
