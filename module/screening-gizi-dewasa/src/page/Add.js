...
import ScreeningGiziDewasaLabel from './Label';

export default function Add() {
  const history = useHistory();
  const formLabel = ScreeningGiziDewasaLabel.formLabel;
  ...
  function getDefaultOptionValue(type) {
    let data = _.filter(ScreeningGiziDewasaLabel[type], 'default');
    if (data.length > 0) {
  ...
  function renderOptionData(type) {
    return ScreeningGiziDewasaLabel[type].map((data) => {
      return {
        key: data.key,
  ...
  return (
    <Fragment>
      <Modal.Header className="text-xl">
        ...
        {ScreeningGiziDewasaLabel.headerLabel}
        <label className="mx-2">&bull;</label>
        {ScreeningGiziDewasaLabel.descriptionLabel}
      </Modal.Header>
          ...
          <Grid.Row columns="equal">
            ...
            <Grid.Column>
              <label className="block">
                <Input
                  fluid
                  placeholder="Tidak ada diagnosis / kondisi khusus"
                />
              </label>
            </Grid.Column>
          </Grid.Row>
