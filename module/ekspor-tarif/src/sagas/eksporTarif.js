...
function* handleSubmitFailure({ payload, meta }) {
  ...
  yield put(
    actions.onFocusElement(meta.resource, getFirstElementError(payload.errors))
  );
}
...
function* handleSearch({ meta, payload }) {
  try {
    ...
    if (response.status) {
      yield put(
        actions.getHeaderKomponen.requestSuccess(
          meta.resource,
          parseToColumnHeader(response.data)
        )
      );
    } else {
      yield put(
        actions.getHeaderKomponen.requestFailure(
          meta.resource,
          response.message
        )
      );
