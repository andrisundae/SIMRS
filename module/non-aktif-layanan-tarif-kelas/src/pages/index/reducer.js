...
const filterReducer = (state = filterState, action) =>
    produce(state, draft => {
        ...
        switch (type) {
            ...
            case filterActionTypes.CHANGE_VERSITARIF: {

                return produce(state, draft => {
                    draft.post.versi_tarif = payload.data ? payload.data.value : 0;
            ...
            case filterActionTypes.CHANGE_KLASIFIKASI: {
                ...
                return produce(state, draft => {
                    draft.post.klasifikasi = payload.data ? payload.data.value : 0;
                    draft.post.selectedKlasifikasi = payload.data;
                    draft.post.kelompok = '';
                    draft.post.selectedKelompok = null;
                    draft.focusElement = '';
                    draft.post.nama_layanan = '';
                })
            }
            ...
            case filterActionTypes.CHANGE_KELAS: {
                ...
                return produce(state, draft => {
                    draft.post.kelas = payload.data ? payload.data.value : 0;
