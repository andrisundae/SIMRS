...
import { actions, actionTypes, selectors } from '../pages/index';
    ...
    if (row.daftarAturan) {
      row.daftarAturan.forEach((aturanAplikasi) => {
        if (aturanAplikasi.nilai.toString().length <= 0) {
          errors[aturanAplikasi.aturan] = [
            `${aturanAplikasi.label} harus diisi!`,
          ];
        }
      });
    }
  ...
  try {
    ...
    if (_.isEmpty(errors)) {
      ...
      if (response.status) {
        ...
        yield put(
          actions.onFocusElement(resource, getFirstElementError(response.data))
        );
      }
    } else {
