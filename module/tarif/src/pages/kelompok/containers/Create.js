...
import { isDisableForm, moduleActions as actions, moduleActionTypes } from '@simrs/main/src/modules/master/nested';
...
class Create extends Component {
    constructor(props) {
        ...
        this._handleBlurNama = this._handleBlurNama.bind(this);

        this.nama_cetak = createRef();
        this.nama = createRef();
        this.aktif = createRef();
        this.st_cito = createRef();
                                ...
                                <Grid.Row className="form-row">
                                    ...
                                    <Grid.Column width="12" className="field">
                                        <Input
                                            ...
                                            onKeyDown={(e) => this._onFocusElement(e, 'nama_cetak')}
                                            onBlur={this._handleBlurNama}
                                        />
                                    </Grid.Column>
                                </Grid.Row>
                                <Grid.Row className="form-row">
                                    <Grid.Column width="4" className="required field">
                                        <label>{t(this._getKey('label.field.nama_cetak'))}</label>
                                    </Grid.Column>
                                    <Grid.Column width="12" className="field">
                                        <Input
                                            name="nama_cetak"
                                            ref={this.nama_cetak}
                                            value={post.nama_cetak}
                                            disabled={isDisableForm}
                                            onChange={this._handleInputChange}
                                ...
                                <Grid.Row className="form-row">
                                    ...
                                    <Grid.Column width="12" className="field">
                                        <Checkbox
                                            ...
                                            disabled={isDisableForm}
                                            onChange={this._handleInputChange}
                                            onKeyDown={(e) => this._onFocusElement(e, 'st_cito')}
                                            inputRef={this.aktif}
                                        ...
                                        <Checkbox
                                            value={post.st_cito}
                                            name="st_cito"
                                            checked={post.st_cito ? true : false}
                                            disabled={isDisableForm}
                                            onChange={this._handleInputChange}
                                            onKeyDown={(e) => this._onFocusElement(e, 'save')}
                                            inputRef={this.st_cito}
                                            label={t(this._getKey('sublabel.field.st_cito'))}
                                        />
                                    </Grid.Column>
                                </Grid.Row>
                            </Grid>
    ...
    _handleBlurNama() {
        ...
        if (post.nama_cetak === '' && post.nama !== '') {
            this.props.action.onChangeInput(this.props.resource, this.props.subResource, { name: 'nama_cetak', value: post.nama });
        }
    }
...
const mapStateToProps = function (state) {
    const { post, statusForm, focusElement, isSubmitted, submitting} = state.nested.module;
