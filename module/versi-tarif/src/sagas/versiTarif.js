...
function* populateForm({ meta }) {
    ...
    try {
        ...
        if (response.status) {
            yield put(getStatusAktifKunjungan.requestSuccess(meta.resource, response.data));
        } else {
            yield put(getStatusAktifKunjungan.requestFailure(meta.resource, response.message));
        }
