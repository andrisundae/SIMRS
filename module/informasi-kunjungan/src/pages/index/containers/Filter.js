...
import { Form, Grid, Segment, Input, Label } from 'semantic-ui-react';
import { Trans } from 'react-i18next';

import { Select, PreviewButton, DatePicker } from '@simrs/components';
...
class Filter extends Component {
  constructor(props) {
    ...
    this.tgl_mulai = createRef();
    this.tgl_selesai = createRef();
  }
  ...
  setFocus() {
    ...
    if (this[focusElement]) {
      if (this[focusElement].current) {
        if (focusElement === 'tgl_mulai' || focusElement === 'tgl_selesai') {
                ...
                <Grid className="form-grid">
                  ...
                  <Grid.Row className="form-row">
                    ...
                    <Grid.Column width="8" className="field">
                      <Input
                        ...
                        onChange={this.inputChangeHandler}
                        onKeyDown={(e) =>
                          this.focusElementHandler(e, 'tgl_mulai')
                        }
                      />
                    </Grid.Column>
                  </Grid.Row>
                  <Grid.Row className="form-row">
                    ...
                    <Grid.Column width="5" className="field">
                      <DatePicker
                        name="tgl_mulai"
                        inputRef={this.tgl_mulai}
                        dateFormat="dd/MM/yyyy"
                        selected={post.tgl_mulai}
                        onChange={(date) =>
                          this.dateTimeChangeHandler('tgl_mulai', date)
                        }
                        selectsStart
                        startDate={post.tgl_mulai}
                        endDate={post.tgl_selesai}
                        onKeyDown={(e) =>
                          this.focusElementHandler(e, 'tgl_selesai')
                        }
                      />
                    </Grid.Column>
                    <Grid.Column
                    ...
                    >
                      <Label circular>{t(this._getKey('s_d'))}</Label>
                    </Grid.Column>
                    <Grid.Column width="5" className="field">
                      <DatePicker
                        name="tgl_selesai"
                        inputRef={this.tgl_selesai}
                        dateFormat="dd/MM/yyyy"
                        selected={post.tgl_selesai}
                        onChange={(date) =>
                          this.dateTimeChangeHandler('tgl_selesai', date)
                        }
                        selectsEnd
                        startDate={post.tgl_mulai}
                        endDate={post.tgl_selesai}
                        minDate={post.tgl_mulai}
