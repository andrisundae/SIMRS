...
import { validator as commonValidator, toastrActions, formatter } from '@simrs/common';
...
function* loadAll({ payload, meta }) {
    ...
    try {
        ...
        yield put(actions.showButtonImportKelas(meta.resource, meta.subResource, data));
    } catch (error) {
        failCallback();
    }
...
function* handleSave({ payload, meta }) {
    ...
    try {
        ...
        let post = {
            ...payload.data,
            tgl_aktif_tarif: formatter.dateFormatDB(payload.data.tgl_aktif_tarif, 'YYYY-MM-DD'),
            jam_aktif_tarif: formatter.dateFormatDB(payload.data.jam_aktif_tarif, 'HH:mm')
        };
...
function* handleAdd({ meta }) {
    try {
        ...
        yield put(getKodePanggil.request(meta.resource, meta.subResource, reference));
        let response = yield call(api.getKodePanggil, reference);
        if (response.status) {
            yield put(getKodePanggil.requestSuccess(meta.resource, meta.subResource, response.data));
        } else {
            yield put(getKodePanggil.requestFailure(meta.resource, meta.subResource, response.message));
        }
        yield put(loaderActions.hide());
        yield put(moduleActions.onFocusElement(meta.resource, meta.subResource, 'kode_panggil'));
    } catch (error) {
        yield put(loaderActions.hide());
        yield put(toastrActions.error(error.message));
    }
...
function* handleChangeTglAktifTarif({ meta }) {
    yield put(moduleActions.onFocusElement(meta.resource, meta.subResource, 'jam_aktif_tarif'));
}

function* handleChangeJamAktifTarif({ meta }) {
    yield put(moduleActions.onFocusElement(meta.resource, meta.subResource, 'aktif'));
}
...
function* populateForm({ meta }) {
    try {
        ...
        if (response.status) {
            yield put(getKelas.requestSuccess(meta.resource, meta.subResource, response.data));
        } else {
            yield put(getKelas.requestFailure(meta.resource, meta.subResource, response.message));
        }
...
function* handleImportKelas({ payload, meta }) {
    ...
    try {
        ...
        if (_.isEmpty(errors)) {
            ...
            if (response.status) {
                yield put(actions.importKelas.requestSuccess(resource, subResource, response));
            } else {
                isError = true;
                errors = response.data;
            }
        ...
        if (isError) {
            yield put(actions.importKelas.requestFailure(resource, subResource, errors));
