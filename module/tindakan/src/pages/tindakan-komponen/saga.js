...
import { loaderActions, datatableActions, datatableActionTypes } from '@simrs/components';
...
function* loadAll({ payload, meta }) {
    ...
    try {
        ...
        if (response.status) {
            ...
            yield put(actions.loadAll.requestSuccess(meta.resource, meta.subResource, response.data));
        } else {
            failCallback();
        }
    } catch (error) {
...
function* handleSave({ payload, meta }) {
    ...
    try {
        ...
        if (totalTarifKomponen > tarifTindakan || totalTarifKomponen < tarifTindakan) {
        ...
        if (isError) {
            yield put(moduleActions.save.requestFailure(resource, subResource, { message }));
        }
...
function* handleSaveSuccess({ payload, meta }) {
    ...
    if (isEdit) {
        yield put(actions.onRedirectTindakan(meta.resource, meta.subResource, { isRedirect: true }));
    } else {
