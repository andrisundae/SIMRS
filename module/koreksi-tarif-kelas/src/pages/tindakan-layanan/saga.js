...
function* populateForm({ meta }) {
    try {
        ...
        if (data.versiTarif || data.klasifikasi || data.kelas) {
            yield put(populateForm.requestSuccess(meta.resource, meta.subResource, data));
        } else {
            yield put(populateForm.requestFailure(meta.resource, meta.subResource, message));
        }
...
function* handleChangeTarif({ meta, payload }) {
    yield put(moduleActions.save.request(meta.resource, meta.subResource, { ...payload.data.post }));
}
...
function* handleSave({ payload, meta }) {
    ...
    try {
        ...
        if (response.status) {
            ...
            yield put(moduleActions.save.requestFailure(resource, subResource, response.data));
        }
        yield put(loaderActions.hide());
    } catch (error) {
        yield put(loaderActions.hide());
...
function* handleFilter({ payload, meta }) {
    ...
    try {
        ...
        if (_.isEmpty(errors) || payload.data.nama_layanan.length <= 0) {
            yield put(actions.filter.onSubmitSuccess(resource, subResource, payload.data));
        } else {
...
function* handleSubmitFailure({ payload, meta }) {
    ...
    yield put(actions.onFocusElement(meta.resource, meta.subResource, 'nama_layanan'));
}
