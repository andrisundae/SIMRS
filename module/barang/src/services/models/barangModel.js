...
const validationRules = (resource) => {
  return {
    ...
    messages: {
      ...
      jenis_barang: {
        required: i18n.t(`${resource}:validator.jenis.required`),
      },
      kelompok_barang: {
        required: i18n.t(`${resource}:validator.kelompok.required`),
      },
      golongan_barang: {
        required: i18n.t(`${resource}:validator.golongan.required`),
      },
      satuan_terkecil: {
        required: i18n.t(`${resource}:validator.satuan_terkecil.required`),
      },
      metode_update_harga_jual: {
        required: i18n.t(`${resource}:validator.metode_update.required`),
      },
      persentase_profit: {
        maxnumber: i18n.t(`${resource}:validator.persentase_profit.max`),
      },
    },
  };
};
