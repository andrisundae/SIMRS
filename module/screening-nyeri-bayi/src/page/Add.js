...
import ScreeningNyeriBayiLabel from './Label';

export default function Add() {
  const history = useHistory();
  const formLabel = ScreeningNyeriBayiLabel.formLabel;

  const initalState = {};
  formLabel.map((v) => {
    let dataDefault = _.filter(ScreeningNyeriBayiLabel[v.default], 'default');
    initalState[v.key] =
      'total' === v.key
        ? 0
  ...
  function getDefaultOptionValue(type) {
    let data = _.filter(ScreeningNyeriBayiLabel[type], 'default');
    if (data.length > 0) {
  ...
  function renderOptionData(type) {
    return ScreeningNyeriBayiLabel[type].map((data) => {
      return {
        key: data.key,
  ...
  return (
    <Fragment>
      <Modal.Header className="text-xl">
        ...
        {ScreeningNyeriBayiLabel.headerLabel}
        <label className="mx-2">&bull;</label>
        {ScreeningNyeriBayiLabel.descriptionLabel}
      </Modal.Header>
      <Modal.Content scrolling>
        <Grid>
            ...
            <Grid.Column>
              <label className="block">
                <Select
                  clearable
                  fluid
                  options={renderOptionData('oksigen')}
                  defaultValue={getDefaultOptionValue('oksigen')}
                  onChange={(e, { value }) =>
                    handleChange(formLabel[0].key, value)
                  }
                />
            ...
            <Grid.Column>
              <label className="block">
                <Select
                  clearable
                  fluid
                  options={renderOptionData('tandaVital')}
                  defaultValue={getDefaultOptionValue('tandaVital')}
                  onChange={(e, { value }) =>
                    handleChange(formLabel[0].key, value)
                  }
                />
            ...
            <Grid.Column>
              <label className="block">
                <Select
                  clearable
                  fluid
                  options={renderOptionData('tidur')}
                  defaultValue={getDefaultOptionValue('tidur')}
                  onChange={(e, { value }) =>
                    handleChange(formLabel[0].key, value)
                  }
                />
          ...
          <Grid.Row columns="equal">
            ...
            <Grid.Column>
              {ScreeningNyeriBayiLabel.intervensi.map((value) => {
                return (
                  <label
                    key={value.key}
