...
import api from '../services/models/settingJumlahLayananHarianUnitLayananModel';
...
function* populateForm({ meta }) {
    try {
        ...
        let data = { find: 'default' };
        yield put(sumberLainActions.populateForm.request(meta.resource, data));
        let response = yield call(api.getDataSumberLain, data);
        if (response.status) {
        ...
        yield put(filterActions.populateForm.request(meta.resource, data));
        let responseFilter = yield call(api.getDataFilter, data);
        if (responseFilter.status) {
            yield put(filterActions.populateForm.requestSuccess(meta.resource, responseFilter.data));
        } else {
            yield put(filterActions.populateForm.requestFailure(meta.resource, responseFilter.message));
...
function* populateFormSuccess({ meta }) {
    yield put(moduleActions.onFocusElement(meta.resource, 'versi_tarif'));
}
...
function* handleChangeFilter({ meta, payload }) {
    try {
        if (payload.data.filter.target) {
            const { filter, value } = payload.data;

            yield put(loaderActions.show());
            let data = { find: filter.name, by: filter.by, value: value ? value : '' };

            if (payload.data.filter.target === 'filter') {
                yield put(filterActions.populateForm.request(meta.resource, data));
                let responseFilter = yield call(api.getDataFilter, data);
                if (responseFilter.status) {
                    yield put(filterActions.populateForm.requestSuccess(meta.resource, responseFilter.data));
                    ...
                    yield put(filterActions.populateForm.requestFailure(meta.resource, responseFilter.message));
                    yield toastr.error('Error', responseFilter.message);
                }
                yield call(handleReloadGrid);
            } else if (payload.data.filter.target === 'dataSumber') {
                yield put(sumberLainActions.populateForm.request(meta.resource, data));
                let response = yield call(api.getDataSumberLain, data);
                if (response.status) {
                    yield put(sumberLainActions.populateForm.requestSuccess(meta.resource, response.data.data_sumber_lain));
                    ...
                    yield put(sumberLainActions.populateForm.requestFailure(meta.resource, response.message));
                    yield toastr.error('Error', response.message);
                }
            }
            yield put(loaderActions.hide());
        } else {
