...
export default function Index() {
  ...
  const sample = tempData.data.skrining_jatuh_dewasa;
      ...
      sample.map((data, idx) => {
        if ('intervensi' !== value.key) {
          cols.push(
            <Table.Cell
              ...
              className={className('w-60', {
                'font-bold':
                  'total' === value.key || 'kesimpulan' === value.key,
            ...
            finalInterpretasi = (
              <ul>
                {tempIntervensiList.map((dt, idt) => {
                  return (
                    <li key={idt} className="ml-5 list-disc">
                      {dt}
                    </li>
                  );
                })}
          ...
          cols.push(
            <Table.Cell
              key={`${index}_${idx}`}
              className={className('w-60 align-top', {
                'font-bold':
                  'total' === value.key || 'kesimpulan' === value.key,
