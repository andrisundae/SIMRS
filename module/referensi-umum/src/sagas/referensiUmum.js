...
import { actions as indexActions, actionTypes as indexActionTypes } from '../pages/index';
...
function* populateForm({ meta }) {
    try {
        ...
        if (response.status) {
            ...
            yield put(indexActions.getOptionsReferensi.requestSuccess(meta.resource, data));
        } else {
            yield put(indexActions.getOptionsReferensi.requestFailure(meta.resource, response.message));
            yield toastr.error(response.message);
        }
        yield put(loaderActions.hide());
    } catch (error) {
