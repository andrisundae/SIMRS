...
export default function Add() {
  ...
  function getDefaultOptionValue(type) {
    let data = _.filter(ScreeningGiziObstetriLabel[type], 'default');
    if (data.length > 0) {
  ...
  function renderOptionData(type) {
    return ScreeningGiziObstetriLabel[type].map((data) => {
      return {
        key: data.key,
            ...
            <Grid.Column>
              <label className="block">
                <Select
                  clearable
                  fluid
                  options={renderOptionData('darah')}
                  defaultValue={getDefaultOptionValue('darah')}
                  onChange={(e, { value }) =>
