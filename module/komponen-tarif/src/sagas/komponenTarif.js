...
import api from '../services/models/komponenTarifModel';
import {
    moduleActionTypes,
    moduleActions,
    filterActionTypes
...
function* populateForm({ meta }) {
    ...
    try {
        ...
        let { getPenanggungJawab } = actions;

        let penanggungJawab = yield call(api.getPenanggungJawab);
        if (penanggungJawab.status) {
            yield put(getPenanggungJawab.requestSuccess(meta.resource, penanggungJawab.data));
        } else {
            yield put(getPenanggungJawab.requestFailure(meta.resource, penanggungJawab.message));
        }
